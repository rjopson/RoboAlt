...
cdef class PyPart:
    ...
    cdef create_derived(Part* ptr):
        ...
        if obj.type is PyPartType.BULKHEAD:
            ...
            return part
        elif obj.type is PyPartType.PARACHUTE:
            part = PyParachute()
            part = PyParachute.create(<Parachute*>ptr)
            return part
...
cdef extern from "part_parachute.h":
    cdef cppclass Parachute:
        Parachute(string name, string comments, Material* material,
        double diameter, double thickness,
        bool mass_override_switch, double mass_override, bool cg_override_switch, double cg_override);
...
cdef class PyParachute(PyPart):
    ...
    @staticmethod
    cdef PyParachute create(Parachute* ptr):
        obj = <PyParachute>PyParachute.__new__(PyParachute)
        obj.ptr = ptr
        obj.ptr_part = <Part*>ptr
        return obj  
