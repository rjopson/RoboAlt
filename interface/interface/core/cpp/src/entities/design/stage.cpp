...
std::vector<PartInstance*> Stage::InstanceList(bool include_stages_above, bool recursive) const {

    std::vector<PartInstance*> flat_list;
    std::vector<Stage*> stages = StageList(include_stages_above);

    for (auto stage : stages) {
        std::vector<PartInstance*> flat_list_stage = stage->instance_root_->Children(recursive);
        flat_list.insert(flat_list.end(), flat_list_stage.begin(), flat_list_stage.end());
    }
    return flat_list;
}
...
double Stage::DragCoefficientFriction(bool include_stages_above, const double& mach_number) {
    ...
    for (auto instance : InstanceList(include_stages_above, true)) {
...
double Stage::DragCoefficientBase(bool include_stages_above, const double& mach_number, const double& area_thrusting) {
    ...
    std::vector<PartInstance*> instanceList = InstanceList(include_stages_above, true);
