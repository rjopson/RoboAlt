...
Simulation::Simulation(const std::string& name, const std::string& comments, Atmosphere* atmosphere,
    ...
    : Flight(name, comments, atmosphere, height_pad, angle_launch_rod, length_launch_rod),
      ...
      phase_current_(Phase::DETECT_LAUNCH),      
...
Simulation::~Simulation() {
    ...
    for (auto sim_stage : stages_) {
        for (auto command : sim_stage->user_commands) {
            DeleteUserCommand(sim_stage->stage, command);
        }
        delete sim_stage->drag_current;
        delete sim_stage;
    }
}
...
void Simulation::AddStage(Stage* stage) {
    ...
    stages_.back()->stage = stage;
    stages_.back()->drag_current = new Drag();
}
...
SimulationUserCommand* Simulation::CreateUserCommand(Stage* stage) {
    ...
    if (sim_stage != nullptr) {
        sim_stage->user_commands.push_back(user_command);
    }
    else {
        delete user_command;
...
void Simulation::DeleteUserCommand(Stage* stage, SimulationUserCommand* user_command) {
    ...
    if (sim_stage != nullptr) {
        ...
        auto it = std::find(sim_stage->user_commands.begin(), sim_stage->user_commands.end(), user_command);
        if (it != sim_stage->user_commands.end()) {
            sim_stage->user_commands.erase(it);
            delete user_command;
        }
    }
}
...
std::vector<Stage*> Simulation::Stages() const {
    ...
    for (auto sim_stage : stages_) {
        result.push_back(sim_stage->stage);
    }
    return result;
}
...
std::vector<Motor*> Simulation::Motors() const {
    ...
    for (auto sim_stage : stages_) {
        result.push_back(sim_stage->motor);
    }
    return result;
}
...
std::vector<SimulationUserCommand*> Simulation::UserCommands(Stage* stage) const {
    ...
    SimulationStage* sim_stage = GetSimulationStage(stage);
    if (sim_stage != nullptr) {
        return sim_stage->user_commands;
    }
    else {
...
void Simulation::Run() {
    ...
    for (auto it = stages_.rbegin(); it != stages_.rend(); ++it) {
        ...
        while (phase_current_ != Phase::GROUND) {            

            Event event;
            std::vector<SolverResult> results_phase;
            
            if (phase_current_ == Phase::DESCENT_DROGUE ||
                phase_current_ == Phase::DESCENT_MAIN) {

                event = RunPhase(results_phase, phase_current_, initial_conditions, time_start, time_end, step_descent_); //calculate current phase
            }
            else {
                event = RunPhase(results_phase, phase_current_, initial_conditions, time_start, time_end, step_ascent_); //calculate current phase
            }

            //Add data from current phase to current stage flight data
            if (phase_current_ != Phase::DETECT_LAUNCH) {
            ...
            phase_current_ = GetNextPhase(event, time_start);			
            ...
            if (phase_current_ == Phase::ASCENT_UNPOWERED_UNSTACKED && !result_next_stage_saved) {
        ...
        phase_current_ = Phase::ASCENT_UNPOWERED_STACKED; 

        //save results for stage
        stage_current_->simulation_data.Populate(results_current_stage);
...
Event Simulation::RunPhase(std::vector<SolverResult>& results, Phase phase, const std::vector<double>& initial_conditions,
    ...
    switch (phase) {
        case Phase::DETECT_LAUNCH: {
            ...
            mass_empty_current_ = stage_current_->stage->MassEmpty(true) + GetMotorMassStagesAbove();
        ...
        case Phase::ASCENT_UNPOWERED_UNSTACKED: {
            ...
            mass_empty_current_ = stage_current_->stage->MassEmpty(false);
            event_function = std::bind(&Simulation::ApogeeDetect, this, std::placeholders::_1);
            event = Event::APOGEE;
            break;
        }
...
double Simulation::AltitudeMainDeploy(SimulationStage* stage) const {
    ...
    for (auto user_command : stage->user_commands) {
...
Drag* Simulation::GetDragStageStacked(SimulationStage* stage) {
    ...
    if (stage->drag_with_stages_above != nullptr) {
        return stage->drag_with_stages_above;
    }
    else {
        *stage->drag_current = stage->stage->DragModel(true, stage->motor->Area(), 0.0, 5.0, 1000);
        return stage->drag_current;
    }    
}

Drag* Simulation::GetDragStageUnstacked(SimulationStage* stage) {

    if (stage->drag_without_stages_above != nullptr) {
        return stage->drag_without_stages_above;
    }
    else {
        *stage->drag_current = stage->stage->DragModel(false, stage->motor->Area(), 0.0, 5.0, 1000);
        return stage->drag_current;
    }
}
...
Drag* Simulation::GetDragDrogueParachute(SimulationStage* stage) {
    ...
    if (stage->drag_drogue != nullptr) {
        return stage->drag_drogue;
    }

    for (auto command : stage->user_commands) {
        if (command->AssignedCommand() == Command::DEPLOY_DROGUE) {
            *stage->drag_current = command->AssignedParachute()->DragModel();
            return stage->drag_current;
        }
    }
    return nullptr;
...
Drag* Simulation::GetDragMainParachute(SimulationStage* stage) {
    ...
    if (stage->drag_main != nullptr) {
        return stage->drag_main;
    }
    ...
    //if no main chute exists, continue to use drogue
    for (auto command : stage->user_commands) {
        if (command->AssignedCommand() == Command::DEPLOY_DROGUE) {
            *stage->drag_current = command->AssignedParachute()->DragModel();
            return stage->drag_current;
...
std::vector<double> Simulation::Eom(SolverResult& results_step, const double& time, const std::vector<double>& state) {
    ...
    bool motor_thrusting = stage_current_->motor->CurrentlyThrusting(time_this_stage);
    double thrust = stage_current_->motor->GetThrust(time_this_stage);
    ...
    double force_sum;
    if (phase_current_ == Phase::DETECT_LAUNCH ||
        phase_current_ == Phase::ASCENT_POWERED ||
        phase_current_ == Phase::ASCENT_UNPOWERED_STACKED ||
        phase_current_ == Phase::ASCENT_UNPOWERED_UNSTACKED) {
    ...
    double mass = mass_empty_current_ + stage_current_->motor->GetMass(time_this_stage);
    double acceleration = (force_sum / mass) - kGravity;

    //SolverResult results_step;
    results_step.phase = phase_current_;
...
bool Simulation::CoastDetect(const SolverResult& result_step) {
    ...
    if (!stage_current_->motor->CurrentlyThrusting(result_step.time-time_motor_lit_)) {
        return true;
    }
    else {
        return false;
